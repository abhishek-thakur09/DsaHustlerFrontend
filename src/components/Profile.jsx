      ...
      <div className="bg-[#0f172a] p-6 rounded-2xl glow-card border border-white/5">
        <div className="flex flex-col items-center">
          <div className="relative group">
            <ProfileUpload />
<div className="absolute inset-0 rounded-full border-2 border-blue-500/50 group-hover:border-blue-500 transition-colors pointer-events-none z-20" />          </div>
          
          <h2 className="text-2xl mt-4 font-bold tracking-tight">{user.name} <span>{user.lastName}</span></h2>
        ...
        <div className="mt-8 space-y-4 pt-6 border-t border-white/5 text-sm text-gray-300">
          <div className="flex justify-between">
             ...
             <span className="bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded text-xs">{user.role || "User"}</span>
          </div>
          ...
          <div className="pt-2">
            ...
            <a href={user.linkedInProfile} className="text-blue-400 hover:underline truncate block text-xs">{user.linkedInProfile || "Not linked"}</a>
          </div>
          <div>
            <p className="text-gray-500 mb-1">GitHub</p>
            <a href={user.githubProfile} className="text-blue-400 hover:underline truncate block text-xs">{user.githubProfile || "Not linked"}</a>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#0f172a] p-8 rounded-2xl glow-card border border-white/5 flex flex-col items-center justify-center">
        ...
        <div className="text-6xl font-black text-white mb-2">
          {stats.totalSolved}
          <span className="text-gray-600 text-2xl font-normal">/{totalproblem}</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-10 w-full">
          <div className="text-center">
            <p className="text-green-500 text-xs font-bold uppercase mb-1">Easy</p>
            <p className="text-xl font-bold">{stats.easy}</p>
          </div>
          <div className="text-center">
            <p className="text-yellow-500 text-xs font-bold uppercase mb-1">Medium</p>
            <p className="text-xl font-bold">{stats.medium}</p>
          </div>
          <div className="text-center">
            <p className="text-red-500 text-xs font-bold uppercase mb-1">Hard</p>
    ...
    <div className="max-w-6xl mx-auto mt-8 bg-[#0f172a] p-8 rounded-2xl glow-card border border-white/5">
      <h3 className="text-gray-400 font-medium mb-8 uppercase tracking-widest text-xs">Submissions</h3>
